...
**Go progress (as of 2026-06-10):** Mastered functions, multiple return values, the `err != nil` error pattern (producer vs consumer). Now learning **structs** — key breakthrough: he understood why structs are useful ("bundle related data"), and crucially, he FINALLY connected the real-world link: "if I define a struct with specs and Terraform applies it, it creates a real server for me through AWS instead of using the console." This is the bridge he was missing. He's now seeing how Go code → Terraform → AWS resources. Solid progress.

**Terraform progress / IMPORTANT calibration:** Syntax-first did NOT work for TF — he ran init/plan/apply/destroy successfully but said it "looks like gibberish" and rated his understanding ~20%. Root cause: **he has no lived experience of the pain IaC solves** — has an AWS CCP (mostly forgotten) but has NEVER worked with servers or IaC. Abstract benefits don't land. What helped: grounding in the **surprise-AWS-bill fear** (CCP holders relate), `terraform destroy` = "no forgotten resources" button, and the blueprint/construction-crew/logbook analogy. LESSON: for TF, teach the WHY and real-world problem viscerally BEFORE syntax; he may need a foundational "what even is a server / EC2" explainer since that base is missing. Completed the change→plan→apply→destroy demo. **Update (2026-06-10):** The "what is a server/EC2" explainer worked perfectly — he immediately saw the connection when told "EC2 instance = virtual computer in a data center with CPU/RAM/ID/state." The Terraform bridge is now clear to him. See [[github-practice-repo]].
